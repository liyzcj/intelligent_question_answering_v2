...
from QA.milvus_operating import (
    create_index,
    create_table,
    drop_milvus_table,
    get_milvus_rows,
...
def get_similar_question_answer(question, client, conn, cursor, bc):
    ...
    if status.code != 0:
        raise RuntimeError("search Question Error")

    if not results:
        raise RuntimeError("Search Result is empty.")

    if results[0][0].distance < 0.8:
        raise RuntimeError("No similar questions in the database.")
    ...
    for result in results[0]:
        ...
        # print(rows)
        if len(rows):
            out_put.append((rows[0][0], result.distance))
    # print(out_put)
    return out_put
